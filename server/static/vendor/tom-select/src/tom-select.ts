...
export default class TomSelect extends MicroPlugin(MicroEvent) {
  ...
  constructor(
    ...
    user_settings: Partial<TomSettings>
  ) {
    super();
  ...
  setup() {
    ...
    addEvent(control_input, 'keypress', (e) =>
      self.onKeyPress(e as KeyboardEvent)
    );
    ...
    const doc_mousedown = (evt: Event) => {
      ...
      if (
        !wrapper.contains(target as HTMLElement) &&
        !dropdown.contains(target as HTMLElement)
      ) {
        if (self.isFocused) {
          self.blur();
        }
  ...
  setActiveItem(item?: TomItem, e?: MouseEvent | KeyboardEvent) {
    ...
    if (
      ...
      (eventName === 'click' && isKeyDown(constants.KEY_SHORTCUT, e)) ||
  ...
  search(query: string): ReturnType<Sifter['search']> {
    ...
    if (query !== self.lastQuery) {
      ...
      result = self.sifter.search(
        query,
        Object.assign(options, { score: calculateScore })
      );
  ...
  canCreate(input: string): boolean {
    return (
      ...
      input.length > 0 &&
      (this.settings.createFilter as TomCreateFilter).call(this, input)
    );
  }
