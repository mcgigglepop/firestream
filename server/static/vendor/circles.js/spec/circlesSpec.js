describe('Circles', function () {
  ...
  describe('Creation', function () {
    ...
    describe('Generated content', function () {
      ...
      it('adds class attributes to the path', function () {
        expect(
          getSVG().getElementsByTagName('path')[0].getAttribute('class')
        ).toEqual('circles-maxValueStroke');
      });
      ...
      it('adds provided class attributes to the path', function () {
        ...
        expect(
          getSVG().getElementsByTagName('path')[0].getAttribute('class')
        ).toEqual('testMaxValueClass');
      });

      it('contains the SVG without animation', function () {
        var dValue = getSVG().getElementsByTagName('path')[1].getAttribute('d');
        expect(dValue).toEqual(
          'M 59.988797973796764 5.000001140776291 A 55 55 0 0 1 92.3939094694214 104.44811165040569 '
        );
      });
      ...
      it('contains the SVG with animation', function () {
        ...
        var dValue = element.firstChild
          .getElementsByTagName('svg')[0]
          .getElementsByTagName('path')[1]
          .getAttribute('d');
        expect(dValue).toEqual(
          'M 59.988797973796764 5.000001140776291 A 55 55 0 0 1 63.396635173034774 5.1049831997356705 '
        );
      });
  ...
  describe('API', function () {
    ...
    it('can update stroke width', function () {
      ...
      expect(
        getSVG().getElementsByTagName('path')[1].getAttribute('stroke-width') ==
          20
      ).toBeTruthy();
    });
    ...
    it('can update colors', function () {
      ...
      expect(
        paths[0].getAttribute('stroke') === '#000' &&
          paths[1].getAttribute('stroke') === '#fff'
      ).toBeTruthy();
    });
    ...
    it('can return HTML representation of a number', function () {
      expect(circle.htmlifyNumber(12.43, 'int', 'float')).toEqual(
        '<span class="int">12</span>.<span class="float">43</span>'
      );
    });
  });
});
