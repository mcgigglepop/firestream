...
(function (f) {
  ...
  function K(a) {
    E === y &&
      (E =
        -1 ===
        I.serializeToString(f.parseXML(A['xl/worksheets/sheet1.xml'])).indexOf(
          'xmlns:r'
        ));
      ...
      if (f.isPlainObject(c)) K(c);
        ...
        if (E) {
          ...
          for (g = d.attributes.length - 1; 0 <= g; g--) {
            ...
            -1 !== n.indexOf(':') &&
              (e.push({ name: n, value: m }), d.removeAttribute(n));
          }
          g = 0;
  ...
  z.ext.buttons.excelFlash = f.extend({}, H, {
    ...
    action: function (a, b, c, d) {
          ...
          for (var h = 0, B = k.length; h < B; h++) {
            ...
            if (null !== k[h] && k[h] !== y && '' !== k[h]) {
              ...
              for (var J = 0, Q = N.length; J < Q; J++)
                if (
                  ((w = N[J]),
                  k[h].match && !k[h].match(/^0\d+/) && k[h].match(w.match))
                ) {
                  x = k[h].replace(/[^\d\.\-]/g, '');
                  w.fmt && (x = w.fmt(x));
                  x = t(e, 'c', {
      ...
      for (D = b.header.length; p < D; p++)
        v.appendChild(
          t(e, 'col', {
            attr: { min: p + 1, max: p + 1, width: O(b, p), customWidth: 1 },
          })
        );
    ...
    action: function (a, b, c, d) {
      ...
      G(
        ...
        JSON.stringify({
          title: F(d, !1),
          message:
            'function' == typeof d.message ? d.message(b, c, d) : d.message,
          colWidth: n.toArray(),
