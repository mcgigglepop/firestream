...
export default class HSSideNav {
  constructor(el, settings) {
    ...
    this.dataSettings = this.$el.hasAttribute('data-hs-navbar-vertical-aside')
      ? JSON.parse(this.$el.getAttribute('data-hs-navbar-vertical-aside'))
      : {};
    ...
    this.isMini = this.$container.classList.contains(
      this.settings.minClass.slice(1)
    );
    this.isCompact = this.$container.classList.contains(
      this.settings.compactClass.slice(1)
    );
  ...
  init() {
    ...
    if (this.settings.autoscrollToActive) {
      ...
      if ($active) {
        if (
          $active.getBoundingClientRect().y >
          document
            .querySelector(this.settings.navbarVertical)
    ...
    this.topLevelElements = collapseElementList.filter(
      (collapseEl) => getParents(collapseEl, '.nav-collapse').length === 1
    );
        ...
        Array.from([
          ...
          $el.addEventListener('mouseleave', (e) => {
            if (
              ...
              return;
            if (
              this.topLevelElements.includes(
                $el.parentElement.querySelector('.nav-collapse')
              )
            ) {
              timeOut = setTimeout(() => {
                collapse.hide();
    ...
    this.collapseList.forEach((collapse) => {
      collapse._element.addEventListener('show.bs.collapse', (e) => {
        const trigeredEl = e.target,
          parentEl = e.target.hasAttribute('hs-parent-area')
            ? document.querySelector(e.target.getAttribute('hs-parent-area'))
            : null;
        ...
        if (
          ($mainContainer.classList.contains(this.settings.minClass.slice(1)) ||
            this.isCompact) &&
          this.topLevelElements.includes(trigeredEl)
        ) {
          e.preventDefault();
        ...
        setTimeout(() => {
          if (
            ($mainContainer.classList.contains(
              ...
              this.isCompact) &&
            parentEl &&
            parentEl.offsetHeight + parentEl.offsetTop > window.innerHeight
          ) {
            const distance =
              parentEl.offsetHeight + parentEl.offsetTop - window.innerHeight;
        ...
        this.collapseList.forEach(function (collapse) {
          ...
          if (
            collapseEl && triggeredArea ? collapseArea === triggeredArea : false
          ) {
            collapse.hide();
      ...
      collapse._element.addEventListener('hide.bs.collapse', (e) => {
        ...
        trigeredEl.querySelectorAll('.nav-collapse').forEach(($menu) => {
          let collapse = this.collapseList.find(
            (collapse) => collapse._element === $menu
          );
    ...
    $sideNavOverlay.classList.add(
      this.settings.toggleInvokerClass.slice(1),
      this.settings.mobileOverlayClass.slice(1)
    );
    ...
    this.$el.addEventListener('transitionend', () => {
      document
        .querySelector(this.settings.mainContainer)
        .classList.remove(this.settings.transitionOnClassName);
    });
    ...
    document
      .querySelectorAll(this.settings.toggleInvokerClass)
  ...
  toggleOnHover(e, menu) {
    let collapse = this.collapseList.find(
      (collapse) =>
        collapse._element.previousElementSibling === e.target &&
        collapse._element === menu
    );
  ...
  showResolutionChecking() {
    if (
      this.$container.classList.contains(this.settings.showClassNames.xs) &&
      window.innerWidth <= 0
    ) {
      return true;
    } else if (
      this.$container.classList.contains(this.settings.showClassNames.sm) &&
      window.innerWidth <= 576
    ) {
      return true;
    } else if (
      this.$container.classList.contains(this.settings.showClassNames.md) &&
      window.innerWidth <= 768
    ) {
      return true;
    } else if (
      this.$container.classList.contains(this.settings.showClassNames.lg) &&
      window.innerWidth <= 992
    ) {
      return true;
    } else if (
      this.$container.classList.contains(this.settings.showClassNames.xl) &&
  ...
  toggleSubMenu($invoker) {
    ...
    let collapseOthers =
      ...
      allExcludeTarget = [
        ...$invoker.parentNode.querySelectorAll(
          `:scope > ${this.settings.hasSubMenuClass}`
        ),
      ].filter(($item) => $item !== $invoker),
      onAction = $mainContainer.classList.contains(
        this.settings.transitionOnClassName
      ),
      topLevel = !$invoker.parentNode.classList.contains(
        this.settings.subMenuClass.slice(1)
      ),
      mini =
        $mainContainer.classList.contains(this.settings.minClass.slice(1)) ||
        $mainContainer.classList.contains(
          this.settings.compactMinClass.slice(1)
        )
          ? true
          : false,
      parentMenu = $invoker;
    ...
    if ((collapseOthers && onAction) || (collapseOthers && topLevel && mini)) {
      allExcludeTarget
        .reduce((acc, $item) => {
          return (acc = [
            ...acc,
            ...$item.querySelectorAll(this.settings.subMenuClass),
          ]);
        }, [])
        .forEach(($item) => {
          $item.style.display = 'none';
          $item.parentNode.classList.remove(
            this.settings.subMenuInvokerActiveClass.slice(1)
          );
        });
    ...
    // Close sub menu with animation
      ...
      while (
        parentMenu.parentNode.classList.contains(
          this.settings.subMenuClass.slice(1)
        )
      ) {
      ...
      if (mini) {
        setTimeout(() => {
          if (
            parentMenu.offsetHeight + parentMenu.offsetTop >
            window.innerHeight
          ) {
            const distance =
              parentMenu.offsetHeight +
              parentMenu.offsetTop -
              window.innerHeight;
    ...
    // Toggle Class
    $invoker.classList.contains(
      this.settings.subMenuInvokerActiveClass.slice(1)
    )
      ? $invoker.classList.remove(
          this.settings.subMenuInvokerActiveClass.slice(1)
        )
      : $invoker.classList.add(
          this.settings.subMenuInvokerActiveClass.slice(1)
        );
    ...
    if ($menu.offsetParent) {
      ...
      document.querySelector('.navbar-vertical-container').addEventListener(
        'scroll',
        () => {
          this.setPosition($menu, $invoker);
        },
  ...
  toggleSidebar() {
    ...
    if (
      (!this.showResolutionChecking() &&
        $mainContainer.classList.contains(this.settings.minClass.slice(1))) ||
      (this.showResolutionChecking() &&
        $mainContainer.classList.contains(this.settings.closedClass.slice(1)))
    ) {
    ...
    if (
      ...
      const collapse = this.collapseList.find(
        (collapse) => collapse._element === $menu
      );
      collapse.hide();
    } else {
      ...
      document
        .querySelectorAll('.nav-collapse-action-mobile')
        .forEach(($item) => {
  ...
  setPosition($menu, $invoker) {
    ...
    setTimeout(() => {
      if ($menu.offsetHeight + $menu.offsetTop > window.innerHeight) {
        const distance =
          $menu.offsetHeight + $menu.offsetTop - window.innerHeight;
