...
export default class HsNavScroller {
    ...
    for (let i = 0; i < that.collection.length; i += 1) {
      ...
      if (_options.type == 'vertical') {
        Velocity(_$el, 'scroll', {
          container: _$el,
          offset:
            _$el.querySelector(_options.target).offsetTop - _options.offset,
        ...
        _options.activeElementLeftPosition = _options.nav.querySelector(
          _options.target
        ).offsetLeft;
        _options.scrollMaxLeft = parseInt(
          (
            _options.nav.scrollWidth.toFixed() - _options.nav.clientWidth
          ).toFixed()
        );
        ...
        if (_options.activeElementLeftPosition > navRect.width / 2) {
          Velocity(_options.nav, 'scroll', {
            ...
            offset:
              _options.activeElementLeftPosition -
              _options.offset -
              prevRect.width,
            duration: _options.delay,
            axis: 'x',
        ...
        _options.next.addEventListener('click', () => {
          Velocity(_options.nav, 'scroll', {
            container: _options.nav,
            offset:
              _options.scrollPosition +
              _options.nav.clientWidth -
              nextRect.width,
            duration: _options.delay,
            axis: 'x',
          });
        ...
        _options.nav.addEventListener('scroll', () => {
          var scrollMaxLeft = (
              parseInt(_options.nav.scrollWidth.toFixed()) -
              parseInt(_options.nav.clientWidth)
            ).toFixed(),
  ...
  onResize($el, settings) {
    const scrollMaxLeft =
        parseInt(settings.nav.scrollWidth.toFixed()) -
        parseInt(settings.nav.clientWidth.toFixed()),
