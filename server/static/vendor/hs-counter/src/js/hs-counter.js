      ...
      const appearSettings = Object.assign({}, this.settings, {
        ...
        appear: (innerEl) => {
          var $item = innerEl,
            ...
            iterationValue = parseInt(
              endValue / (_options.time / _options.fps),
              10
            );
          ...
          $item.data = {
            intervalId: setInterval(() => {
              if (_options.isCommaSeparated) {
                $item.innerHTML = this._getCommaSeparatedValue(
                  (counter += iterationValue)
                );
              } else if (_options.isReduceThousandsTo) {
                $item.innerHTML = this._getCommaReducedValue(
                  (counter += iterationValue),
                  _options.isReduceThousandsTo
                );
              } else {
