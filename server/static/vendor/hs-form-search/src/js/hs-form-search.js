...
export default class HSFormSearch {
  ...
  _init() {
    ...
    for (let i = 0; i < that.collection.length; i += 1) {
      ...
      _options.$dropMenuElement = document.querySelector(
        _options.dropMenuElement
      );
      ...
      _options.$clearIcon.addEventListener('click', () => {
        ...
        if (
          Object.prototype.hasOwnProperty.call(
            that.collection[i].$initializedEl.events,
            'clear'
          )
        ) {
      ...
      if (_options.$dropMenuElement) {
        _options.$dropMenuElement.classList.add(
          'animated',
        ...
        document.addEventListener('click', (e) => {
          ...
          if (
            (_$el !== e.target && e.target.closest('a')) ||
            (_$el !== e.target &&
              window.getComputedStyle(_options.$dropMenuElement).display ===
                'block' &&
              !e.target.closest(_options.dropMenuElement))
          ) {
            ...
            if (
              ...
              that.collection[i].$initializedEl.events.close(
                _options.$dropMenuElement
              );
            }
        ...
        _$el.addEventListener('click', () => {
          if (_options.$dropMenuElement.style.display !== 'block') {
            setTimeout(() => {
              _options.$dropMenuElement.style.top =
                100 + _options.dropMenuOffset + '%';
              _options.$dropMenuElement.style.width = '100%';
              _options.$dropMenuElement.style.animationDuration =
                _options.dropMenuDuration + 'ms';

              _options.$dropMenuElement.classList.remove(
                'hs-form-search-menu-hidden',
                'fadeOut'
              );
    ...
    if (!settings.isLoading) {
      ...
      if (!settings.$defaultIcon) {
        if (input) {
          fadeIn(
            settings.$clearIcon,
            settings.$loadingIcon ? 10 : settings.delay
          );
