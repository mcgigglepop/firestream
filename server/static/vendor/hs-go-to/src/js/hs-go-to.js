...
export default class HSGoTo {
  ...
  _init() {
    ...
    for (let i = 0; i < that.collection.length; i += 1) {
      ...
      const _compensationSelector = document.querySelector(
          _options.compensationSelector
        ),
        _targetSelector = document.querySelector(_options.targetSelector),
        _pageContainerSelector = document.querySelector(
          _options.pageContainerSelector
        );

      _options.targetOffsetTop = () => {
        if (_compensationSelector) {
          return _targetSelector
            ? _targetSelector.offsetTop - _compensationSelector.innerHeight
            : 0;
        } else {
      ...
      _$el.addEventListener('click', (e) =>
        this.clickEvents(_$el, _options, {
          _pageContainerSelector,
          _compensationSelector,
          _targetSelector,
        })
      );
      ...
      if (_options.animationIn && _options.animationOut) {
        document.addEventListener('scroll', (e) =>
          this.scrollEvents(_$el, _options)
        );
      }
    }
  }
  ...
  prepareObject($el, settings) {
    if (settings.animationIn && settings.animationOut) {
      ...
      $el.classList.add(
        settings.animationInit,
        settings.animationOut,
        settings.preventEventClass
      );
    }
  }
