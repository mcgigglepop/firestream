...
export default class HSToggleSwitch {
      ...
      if (_options.mode === 'toggle-count') {
        if (_options.isChecked) {
          ...
          _options.$targets.forEach(($target) => {
            const currentDataSettings = $target.hasAttribute(
              dataAttributeItemName
            )
              ? JSON.parse($target.getAttribute(dataAttributeItemName))
              : {};
  ...
  _countUpEach(settings) {
    ...
    settings.$targets.forEach(($target) => {
      const currentDataSettings = $target.hasAttribute(dataAttributeItemName)
        ? JSON.parse($target.getAttribute(dataAttributeItemName))
        : {};
