...
QUnit.test('Improperly closed elements', function (assert) {
  ...
  expectWarning(
    assert,
    'Elements not self-closable nested wrong',
    4,
    function () {
  ...
  expectNoWarning(
    assert,
    "Script doesn't run in processed strings",
    function () {
      jQuery("<script>alert( 'YOU SHOULD NOT SEE THIS ALERT' )</script>");
      jQuery('<div>').append(
        "<script src='YOU-SHOULD-NOT-SEE-THIS.SCRIPT'></script>"
      );
  ...
  expectNoWarning(
    assert,
    'Elements not self-closable but tolerable',
    function () {
...
QUnit.test(
  'jQuery.UNSAFE_restoreLegacyHtmlPrefilter (deprecated)',
  function (assert) {
    assert.expect(5);
