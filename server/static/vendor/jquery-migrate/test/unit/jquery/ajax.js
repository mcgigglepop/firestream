...
if (jQuery.ajax) {
  ...
  QUnit.test('jQuery.ajax() deprecations on jqXHR', function (assert) {
    ...
    expectWarning(
      assert,
      '.success(), .error(), .complete() calls',
      3,
      function () {
        return jQuery
          .ajax(url('not-found.404'))
          .success(jQuery.noop)
    ...
    QUnit[jQueryVersionSince('4.0.0') ? 'skip' : 'test'](
      ...
      function (assert) {
        ...
        var done = assert.async(),
          tests = [
            function () {
              return expectNoWarning(assert, 'dataType: "json"', function () {
                return jQuery
                  .ajax({
                    url: url('null.json'),
