...
QUnit.test('.offset()', function (assert) {
  ...
  expectWarning(assert, '.offset() on window', function () {
    assert.strictEqual(jQuery(window).offset(), undefined, 'window undefined');
  });
  ...
  expectNoWarning(assert, '.offset() on disconnected node', function () {
    assert.deepEqual(
      ...
      bogus,
      'disconnected bogus top/left 0'
    );
  });

  expectNoWarning(
    assert,
    '.offset() as setter on disconnected node',
    function () {
      var offset,
        $elemInitial = jQuery('<div />').css('position', 'fixed'),
        $elem = $elemInitial.offset({ top: 42, left: 99 });
