...
QUnit.test("jQuery( '#' )", function (assert) {
  ...
  expectWarning(
    assert,
    'Selector, through the jQuery constructor, nothing but hash',
    function () {
      var set = jQuery('#');
...
QUnit.test('Attribute selectors with unquoted hashes', function (assert) {
  ...
  var markup = jQuery(
    ...
    failures = ['p[class ^= #junk]:first', 'a[href=space#junk]:eq(1)'];

  expectNoWarning(
    assert,
    'Perfectly cromulent selectors are unchanged',
    function () {
  ...
  expectWarning(
    assert,
    'Values with unquoted hashes are quoted',
    fixables.length * 2,
    function () {
  ...
  expectNoWarning(
    assert,
    'ready function with attribute selector',
    function () {
      try {
        jQuery(function () {
...
QUnit.test(
  'XSS injection (XSS with hash and leading space)',
  function (assert) {
    assert.expect(2);
    ...
    assert.equal(
      threw,
      true,
      'Throw on leading-hash HTML and space (treated as selector)'
    );
    assert.equal(window.XSS, false, 'XSS');
  }
);
...
QUnit.test(
  "jQuery( '<element>' ) usable on detached elements (#128)",
  function (assert) {
    assert.expect(1);
...
QUnit.test('isNumeric', function (assert) {
  ...
  assert.equal(
    t(new ToString('42')),
    false,
    'Only limited to strings and numbers'
  );
  ...
  assert.equal(
    t('7.2acdgs'),
    false,
    'Number with trailing non-numeric characters'
  );
...
QUnit[typeof Symbol === 'function' ? 'test' : 'skip'](
  ...
  function (assert) {
    ...
    assert.equal(
      jQuery.isNumeric(Object(Symbol())),
      false,
      'Symbol inside an object'
    );
  }
);

QUnit[jQueryVersionSince('3.3.0') ? 'test' : 'skip'](
  '.isNumeric (warn)',
  function (assert) {
    assert.expect(3);
    ...
    expectWarning(
      assert,
      'warning on isNumeric (and possibly type)',
      function () {
...
QUnit[jQueryVersionSince('3.1.1') ? 'test' : 'skip'](
  'jQuery.trim',
  function (assert) {
    assert.expect(14);
...
QUnit[jQueryVersionSince('3.2.0') ? 'test' : 'skip'](
  'jQuery.nodeName',
  function (assert) {
    assert.expect(2);
...
QUnit[jQueryVersionSince('3.3.0') ? 'test' : 'skip'](
  ...
  function (assert) {
    ...
    assert.equal(
      jQuery.type(document.createTextNode('foo')),
      'object',
      'TextNode'
    );
    assert.equal(
      jQuery.type(document.getElementsByTagName('*')),
      'object',
      'NodeList'
    );
