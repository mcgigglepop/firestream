TestManager = {
  ...
  loadProject: function (projectName, defaultVersion, isSelf) {
    var file,
      ...
      projectRoot =
        this.baseURL + (isSelf ? '../..' : '../../../' + projectName),
      version =
        (matcher.exec(document.location.search) || {})[1] || defaultVersion;
  ...
  init: function (projects) {
    var p,
      project,
      originalDeduplicateWarnings,
      disabledPatches,
      origMigrateDisablePatches,
      FILEPATH = '/test/data/testinit.js',
      activeScript = [].slice.call(
...
TestManager.init({
  jquery: {
    urlTag: 'jquery',
    choices:
      'dev,min,git,git.min,git.slim,git.slim.min,' +
...
(function () {
  ...
  url = decodeURIComponent(
    url.slice(url.indexOf('swarmURL=') + 'swarmURL='.length)
  );
  ...
  document.write(
    '<scr' +
      "ipt src='https://swarm.jquery.org/js/inject.js?" +
      new Date().getTime() +
      "'></scr" +
      'ipt>'
  );
