...
function initialize(next) {
  ...
  nextVersion =
    major + '.' + minor + '.' + (isBeta ? patch : +patch + 1) + '-pre';
  next();
}
...
function tagReleaseVersion(next) {
  git(
    [
      ...
      '--no-verify',
      '-m',
      'Tagging the ' + releaseVersion + ' release.',
    ],
    function () {
...
async function publishToNPM(next) {
  ...
  if (isBeta) {
    exec(
      npmCmd,
      ['publish', '--tag', 'beta', ...(otp ? ['--otp', otp] : [])],
      next,
      skipRemote
    );
  } else {
    exec(npmCmd, ['publish', ...(otp ? ['--otp', otp] : [])], next, skipRemote);
  }
}
...
function setNextVersion(next) {
  ...
  git(
    [
      ...
      '--no-verify',
      '-m',
      'Updating the source version to ' + nextVersion,
    ],
    next
  );
}
...
function remindAboutCDN(next) {
  console.log(
    chalk.red(
      'TODO: Update CDN with jquery-migrate.' +
        releaseVersion +
        ' files (min and regular)'
    )
  );
  console.log(
    chalk.red('  clone codeorigin.jquery.com, git add files, commit, push')
  );
  next();
}
...
function remindAboutSites(next) {
  console.log(
    chalk.red('TODO: Update jquery.com download page to ' + releaseVersion)
  );
  next();
}
