...
export default class HSAddField {
      ...
      _$el.addEventListener('click', (e) => {
        if (e.target.closest(_options.createTrigger)) {
          ...
          that.deleteField(
            ...
            _options,
            e.target
              .closest(_options.deleteTrigger)
              .getAttribute(_options.flags.delete)
          );
        }
      });
    }
  ...
  deleteField($el, settings, index) {
    ...
    if (settings.fieldsCount > 0) {
      settings.contaienr.childNodes[index].parentNode.removeChild(
        settings.contaienr.childNodes[index]
      );
  ...
  renderName($el, settings) {
    settings.contaienr.childNodes.forEach((el, key) => {
      ...
      field.setAttribute(
        'name',
        `${field.getAttribute('data-name')}${settings.nameSeparator}${key}`
      );
    });
  }
  ...
  renderKeys($el, settings) {
    settings.contaienr.childNodes.forEach((el, key) => {
      ...
      deleteTrigger
        ? deleteTrigger.setAttribute(settings.flags.delete, key)
        : null;
    });
  }
