...
export default class HSHeaderShowHideObserver extends HSAbstractObserver {
  ...
  init() {
    ...
    this.transitionDuration =
      parseFloat(getComputedStyle(this.element)['transition-duration'], 10) *
      1000;

    this.offset =
      isFinite(this.dataSettings.fixMoment) &&
      this.dataSettings.fixMoment > this.element.offsetHeight
        ? this.dataSettings.fixMoment
        : this.element.offsetHeight + 100;
    this.effect = this.dataSettings.fixEffect
      ? this.dataSettings.fixEffect
      : 'show-hide';
