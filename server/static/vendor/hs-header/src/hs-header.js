...
export default class HSHeader {
  constructor(el, config, observers) {
    ...
    this.observers =
      observers &&
      Object.prototype.toString.call(observers) === '[object Object]'
        ? observers
        : {};
    this.viewport = 'xs';
    this.defaults = {
  ...
  init() {
    ...
    let dataSettings = element.hasAttribute('data-hs-header-options')
      ? JSON.parse(element.getAttribute('data-hs-header-options'))
      : {};
    ...
    function onScroll() {
      ...
      if (
        window.pageYOffset < self.config.fixMoment - 100 &&
        self.config.effectCompensation === true
      ) {
  ...
  _detectObservers() {
    ...
    if (this.element.classList.contains('navbar-has-hidden-element')) {
      observers['xs'].push(new HSHeaderHasHiddenElement(this.element).init());
    }
    ...
    if (this.element.classList.contains('navbar-sticky-top')) {
      ...
      observers['xs'].push(new HSHeaderStickObserver(this.element).init());
    }

    // Floating
    if (this.element.classList.contains('navbar-floating')) {
      observers['xs'].push(new HSHeaderFloatingObserver(this.element).init());
    }
    ...
    // Abs top & Static
    if (
      this.element.classList.contains('navbar-absolute-top') ||
      this.element.classList.contains('navbar-static')
    ) {
      if (this.element.classList.contains('navbar-show-hide')) {
        observers['xs'].push(
  ...
  fixMediaDifference(element) {
    if (
      !element ||
      !element.length ||
      !element.filter((el) => el.closest('[class*="navbar-side"]')).length
    )
      return;
    ...
    if (
      element.classList.contains('navbar-side-left-xl') ||
      element.classList.contains('navbar-side-right-xl')
    ) {
      ...
      element.classList.contains('navbar-side-left-lg') ||
      element.classList.contains('navbar-side-right-lg')
    ) {
      ...
      element.classList.contains('navbar-side-left-md') ||
      element.classList.contains('navbar-side-right-md')
    ) {
      ...
      element.classList.contains('navbar-side-left-sm') ||
      element.classList.contains('navbar-side-right-sm')
    ) {
  ...
  checkViewport() {
    if (
      window.innerWidth > this.config.breakpointsMap['sm'] &&
    ...
    if (
      window.innerWidth > this.config.breakpointsMap['md'] &&
    ...
    if (
      window.innerWidth > this.config.breakpointsMap['lg'] &&
    ...
    if (
      window.innerWidth > this.config.breakpointsMap['xl'] &&
