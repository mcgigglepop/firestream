...
This repository contains a real-time analytics pipeline designed to process and store game telemetry data, such as player events (kills, wins, items collected), using a scalable architecture powered by **AWS Kinesis**, **AWS CDK**, **Golang**, and **DynamoDB**. The pipeline enables real-time leaderboards, providing up-to-date insights into player performance.
...
- **Golang**: Backend service for processing player events and interacting with Kinesis and DynamoDB.
- **DynamoDB**: Managed NoSQL database to store game telemetry and leaderboards data.
