...
def send_record_batch(kinesis_client, stream_name, raw_records):
    ...
    for rec in raw_records:
        formatted_records.append(
            {'PartitionKey': rec['event']['event_id'], 'Data': json.dumps(rec)})
    kinesis_client.put_records(
        StreamName=stream_name, Records=formatted_records)
    print('Sent %d records to stream %s.' %
          (len(formatted_records), stream_name))
...
def send_events_bulk(kinesis_client, stream_name, batch_size, application_id):
    """Send a batches of randomly generated events to Amazon Kinesis."""
...
def send_data(params):
    ...
    kinesis_stream = params['stream_name']
    ...
    send_events_bulk(client, kinesis_stream, batch_size, application_id)


# Set Global value for Server and Match id
